...
//ray
Player.prototype.getPL = function getPL(id, limit, offset) {
  const playlistIdentifier = 'SQ:' + id.toString();
  limit = limit || 0;
  offset = offset || 0;

  if (!limit) {
    return this.browseAll(playlistIdentifier);
  }

  return this.browse(playlistIdentifier, offset, limit)
    .then(result => result.items);
};
